...
The MongrelDB C++ client is a header-only C++17 library that talks to
`mongreldb-server` over HTTP using libcurl. The client itself holds no
...
- The bundled JSON parser is strict about structure and throws
  `QueryException` on malformed input rather than reading out of bounds.
- All client state is RAII-managed: the destructor releases libcurl resources.
  There are no caller-side free obligations.
...
- Server and network errors are mapped to the typed exception hierarchy
  ...
  `QueryException`), not leaked as generic `std::exception`s.
...
through GitHub's Dependabot alerts or the private vulnerability reporting flow
below.
